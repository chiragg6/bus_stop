...
func getDirection(route, direction string) (string, error) {
	...
	// fmt.Println(directionList)
	for _, directionData := range directionList {
		// fmt.Println(directionMap[strings.ToLower(direction)], directionData.Value)
...
func getStops(route, direction, busStop string) (string, error) {
	...
	for _, stop := range stopList {
		if strings.Compare(strings.ToLower(busStop), strings.ToLower(strings.TrimPrefix(strings.TrimSuffix(stop.Text, " "), " "))) == 0 {
...
func getTimeDepartures(route, direction, stop string) (float64, error) {
	...
	if len(timeDepList) <= 0 {
		return 0, errors.New("bus list is empty")
	}
